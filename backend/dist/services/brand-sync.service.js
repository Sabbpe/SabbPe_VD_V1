...
exports.brandSyncService = exports.BrandSyncService = void 0;
const supabase_1 = require("../config/supabase");
const valuedesign_service_1 = require("./valuedesign.service");
...
class BrandSyncService {
    ...
    async syncBrands() {
        try {
            ...
            logger_1.logger.info('🔄 Starting brand sync from ValueDesign...');
            logger_1.logger.info('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            ...
            if (!vdBrands || vdBrands.length === 0) {
                logger_1.logger.warn('⚠️ No brands returned from ValueDesign');
                return { success: false, count: 0, error: 'No brands found' };
            }
            logger_1.logger.info(`✓ Fetched ${vdBrands.length} brands from ValueDesign`);
            // Transform VD brands to our schema - FIXED TYPE
            const brandRecords = vdBrands.map((vdBrand) => {
                return this.transformVDBrandToCache(vdBrand);
            });
            logger_1.logger.info(`✓ Transformed ${brandRecords.length} brands`);
            // Delete all existing brands (fresh sync)
            logger_1.logger.info('🗑️ Clearing existing brand cache...');
            ...
            if (deleteError) {
                ...
                logger_1.logger.error('Failed to clear brand cache:', {
                    message: deleteError.message,
                    code: deleteError.code,
                });
            }
            else {
                logger_1.logger.info('✓ Brand cache cleared');
            }
            ...
            for (let i = 0; i < brandRecords.length; i += batchSize) {
                ...
                logger_1.logger.info(`📥 Inserting batch ${Math.floor(i / batchSize) + 1}: ${batch.length} brands`);
                const { data, error: insertError } = await supabase_1.supabase
                    .from('brand_cache')
                    .insert(batch)
                    .select();
                if (insertError) {
                    logger_1.logger.error(`Failed to insert batch ${Math.floor(i / batchSize) + 1}:`, {
                ...
                logger_1.logger.info(`✓ Batch inserted: ${data?.length} brands`);
            }
            logger_1.logger.info('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            logger_1.logger.info(`✅ Brand sync completed: ${insertedCount}/${vdBrands.length} brands synced`);
            logger_1.logger.info('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            return { success: true, count: insertedCount };
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            logger_1.logger.error('❌ Brand sync failed:', { error: errorMsg });
     ...
     */
    transformVDBrandToCache(vdBrand) {
        ...
        if (typeof vdBrand.Images === 'string') {
            try {
                ...
                logger_1.logger.warn(`Failed to parse images for ${vdBrand.BrandCode}`);
    ...
    async getCachedBrands(filters) {
        try {
            ...
            if (filters?.search) {
                query = query.or(`brand_name.ilike.%${filters.search}%,brand_code.ilike.%${filters.search}%`);
            }
            const { data, error } = await query;
            if (error) {
                // FIX: Proper error logging
                logger_1.logger.error('Failed to fetch cached brands:', {
            ...
            // FIX: Proper error logging
            logger_1.logger.error('Error fetching cached brands:', {
    ...
    async getBrandByCode(brandCode) {
        try {
            ...
            if (error) {
                // FIX: Proper error logging
                logger_1.logger.error(`Brand not found: ${brandCode}`, {
                    message: error.message,
                    code: error.code,
                });
                return null;
            ...
            // FIX: Proper error logging
            logger_1.logger.error('Error fetching brand by code:', {
                error: error instanceof Error ? error.message : String(error),
            });
            return null;
        }
...
exports.BrandSyncService = BrandSyncService;
exports.brandSyncService = new BrandSyncService();
//# sourceMappingURL=brand-sync.service.js.map