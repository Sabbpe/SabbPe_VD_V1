...
class ValueDesignService {
    ...
    */
     ...
     */
    /**
 * Get stores for a brand with geocoding support
 * @param brandCode - The brand code to fetch stores for
 * @param forceRefresh - If true, bypass cache and fetch fresh data
 * @param includeGeocoding - If true, geocode addresses (slower but adds lat/lng)
 */
    async getStores(brandCode, forceRefresh = false, includeGeocoding = false) {
        try {
            logger_1.logger.info('Fetching stores for brand', { brandCode, forceRefresh, includeGeocoding });
            // 1. Check cache first (unless force refresh)
            if (!forceRefresh) {
                const cachedStores = await this.getStoresFromCache(brandCode);
                if (cachedStores && cachedStores.length > 0) {
                    logger_1.logger.info('✓ Returning cached stores', {
                        brandCode,
            ...
            // 2. Fetch from ValueDesign API
            const token = await this.generateToken();
            const requestPayload = { BrandCode: brandCode };
            logger_1.logger.info('🚀 Sending request to VD Store API:', {
                url: `${VD_CONFIG.baseUrl}/distributor/api-getstore/`,
                payload: requestPayload,
            });
            const response = await this.client.post('/distributor/api-getstore/', requestPayload, {
                headers: { token: token },
            });
            logger_1.logger.info('📥 VD Store API Response:', {
                httpStatus: response.status,
            ...
            if (!isSuccess) {
                const errorCode = response.data.responseCode || response.data.statuscode;
                const errorMessage = (0, vd_error_codes_1.getVDErrorMessage)(errorCode);
                logger_1.logger.error('❌ ValueDesign Store API Error:', {
                    errorCode: errorCode,
                    errorMessage: errorMessage,
                    responseMsg: response.data.responseMsg,
                });
            ...
            // 4. Decrypt data
            if (!response.data.data) {
                logger_1.logger.warn('⚠️ No store data returned for brand', { brandCode });
                return [];
            }
            let stores = [];
            try {
                ...
                else if (parsedData.stores && Array.isArray(parsedData.stores)) {
                    stores = parsedData.stores;
                }
                else if (parsedData.data && Array.isArray(parsedData.data)) {
                    stores = parsedData.data;
                }
                else {
                    stores = [parsedData];
                }
                ...
                logger_1.logger.error('Failed to decrypt store data:', {
                    error: String(decryptError),
                });
                throw new Error('Failed to decrypt store data');
            }
            // 5. ENHANCEMENT: Extract pincodes and geocode if requested
            if (includeGeocoding) {
                logger_1.logger.info('🗺️ Starting geocoding process...');
                const enhancedStores = await Promise.all(stores.map(async (store, index) => {
                    const pincode = (0, addressParser_1.extractPincode)(store.Address || '');
                    if (!pincode) {
                        logger_1.logger.warn(`Store ${index + 1} has no pincode`, {
                            city: store.City,
                            address: store.Address?.substring(0, 50)
                        });
                        return { ...store, pincode: null };
                    }
                    // Geocode the pincode
                    const coords = await (0, geocoding_1.getCoordinatesFromPincode)(pincode);
                    if (coords) {
                        logger_1.logger.info(`✓ Geocoded store ${index + 1}/${stores.length}`, {
                            pincode,
                            lat: coords.latitude,
                        ...
                        return {
                            ...
                            geocoded_city: coords.city,
                            geocoded_state: coords.state
                        };
                    }
                    else {
                        logger_1.logger.warn(`Failed to geocode pincode: ${pincode}`);
                        return { ...store, pincode };
                    }
                }));
                stores = enhancedStores;
                logger_1.logger.info('✓ Geocoding complete', {
                    total: stores.length,
                    geocoded: stores.filter(s => s.latitude).length
                });
            }
            else {
                // Just extract pincodes without geocoding
                stores = stores.map(store => ({
                    ...store,
                    pincode: (0, addressParser_1.extractPincode)(store.Address || '')
                }));
            }
            // 6. Cache stores in database
            if (stores.length > 0) {
                await this.cacheStores(brandCode, stores);
            }
            return stores;
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            logger_1.logger.error('Failed to fetch stores:', {
                error: errorMessage,
                brandCode
            });
            throw error;
    ...
    /**
     * Enhanced cache method with geocoding support
     */
    async cacheStores(brandCode, stores) {
        try {
            // 1. Deactivate old stores for this brand
            const { error: deactivateError } = await supabase_1.supabase
                .from('store_cache')
                .update({ is_active: false, updated_at: new Date().toISOString() })
                .eq('brand_code', brandCode);
            if (deactivateError) {
                logger_1.logger.error('Failed to deactivate old stores:', { error: deactivateError.message });
            }
            // 2. Prepare store data for insertion
            const storesToInsert = stores.map((store, index) => {
                const storeCode = `${brandCode}-${store.City || 'UNKNOWN'}-${String(index + 1).padStart(3, '0')}`;
                return {
                    store_code: storeCode,
                    store_name: store.StoreName || store.BrandName || 'Unknown Store', // ← ADD THIS
                    ...
                    catchment_pincodes: store.pincode ? [store.pincode] : [],
            ...
            if (insertError) {
                logger_1.logger.error('Failed to cache stores:', { error: insertError.message });
                throw insertError;
            }
            logger_1.logger.info('✓ Stores cached successfully', {
                brandCode,
                count: storesToInsert.length,
                geocoded: storesToInsert.filter(s => s.latitude).length
            });
        }
        catch (error) {
     ...
     */
    async getStoresFromCache(brandCode) {
        try {
            const { data, error } = await supabase_1.supabase
                .from('store_cache')
                .select('*')
                ...
                .eq('is_active', true);
            if (error) {
                logger_1.logger.error('Failed to fetch stores from cache:', { error: error.message });
                return null;
            }
            return data && data.length > 0 ? data : null;
        }
        catch (error) {
            logger_1.logger.error('Failed to fetch stores from cache:', {
                error: error instanceof Error ? error.message : String(error),
            });
            return null;
        }
