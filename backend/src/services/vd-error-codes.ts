export const VD_ERROR_CODES: Record<number | string, string> = {
    0: 'APPROVAL',
...
export function getVDErrorMessage(code: number | string | undefined): string {
    if (code === undefined || code === null) return 'Unknown error';
    return VD_ERROR_CODES[code] || `Unknown error code: ${code}`;
}
